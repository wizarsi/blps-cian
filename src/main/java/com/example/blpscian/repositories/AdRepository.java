...
public interface AdRepository<T extends Ad> extends JpaRepository<Ad, Long> {
    @Query("SELECT e FROM Ad e WHERE e.adType = :adType AND e.location = (SELECT l FROM Location l WHERE l.address = :address) AND e.price >=:priceMin AND e.price <=:priceMax")
